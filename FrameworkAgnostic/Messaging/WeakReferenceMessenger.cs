...
using FrameworkAgnostic.Messaging.Internals.System.Runtime.CompilerServices;
...
public sealed class WeakReferenceMessenger : IMessenger
{
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        lock (recipientsMap)
        {
            ...
            if (!recipientsMap.TryGetValue(type2, out var table)) return false;

            // Special case for unit tokens
            if (typeof(TToken) == typeof(Unit)) return table.TryGetValue(recipient, out _);
    ...
    public void UnregisterAll(object recipient)
    {
        ...
        lock (recipientsMap)
        {
            var enumerator = recipientsMap.GetEnumerator();
            ...
            while (enumerator.MoveNext()) _ = enumerator.GetValue().Remove(recipient);
        }
    }
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        lock (recipientsMap)
        {
            ...
            if (recipientsMap.TryGetValue(type2, out var value))
            {
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        lock (recipientsMap)
        {
            ...
            if (!recipientsMap.TryGetValue(type2, out var table)) return message;
        ...
        where TToken : IEquatable<TToken>
    {
        lock (recipientsMap)
        {
            ...
            ref var mapping = ref recipientsMap.GetOrAddValueRef(type2);
            ...
            if (typeof(TToken) == typeof(Unit))
            {
                if (!mapping.TryAdd(recipient, dispatcher)) ThrowInvalidOperationExceptionForDuplicateRegistration();
            }
            else
            {
                // Get or create the handlers dictionary for the target recipient
                var map = Unsafe.As<Dictionary2<TToken, object?>>(mapping.GetValue(recipient,
    ...
    {
        object lockObject = recipientsMap;
        var lockTaken = false;
        ...
        try
        {
            ...
            if (lockTaken) CleanupWithoutLock();
        }
        finally
        {
    ...
    {
        using var type2s = ArrayPoolBufferWriter<Type2>.Create();
        using var emptyRecipients = ArrayPoolBufferWriter<object>.Create();

        var type2Enumerator = recipientsMap.GetEnumerator();
        ...
        while (type2Enumerator.MoveNext())
        {
            ...
            else
            {
                ...
                foreach (var recipient in emptyRecipients.Span) _ = type2Enumerator.GetValue().Remove(recipient);
            }

            // Track the type combinations with no recipients or handlers left
            if (!hasAtLeastOneHandler) type2s.Add(type2Enumerator.GetKey());
        }

        // Remove all the mappings with no handlers left
        foreach (var key in type2s.Span) _ = recipientsMap.TryRemove(key);
    }
