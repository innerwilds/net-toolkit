...
public sealed class StrongReferenceMessenger : IMessenger
{
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        lock (recipientsMap)
        {
            ...
            {
                if (!TryGetMapping<TMessage>(out var mapping)) return false;
            ...
            {
                if (!TryGetMapping<TMessage, TToken>(out var mapping)) return false;

                Recipient key = new(recipient);

                return
                    mapping.TryGetValue(key, out var handlers) &&
    ...
    public void UnregisterAll(object recipient)
    {
        ...
        lock (recipientsMap)
        {
            ...
            if (!recipientsMap.TryGetValue(key, out var set)) return;
            ...
            foreach (var mapping in set)
                if (mapping.TryRemove(key) &&
                    ...
                    _ = typesMap.TryRemove(mapping.TypeArguments);

            // Remove the associated set in the recipients map
            _ = recipientsMap.TryRemove(key);
        }
    }
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        if (typeof(TToken) == typeof(Unit)) throw new NotImplementedException();
        ...
        try
        {
            ...
            {
                var handlersMap = Unsafe.As<IDictionary2<Recipient, IDictionary2<TToken>>>(obj);
                    ...
                    holder.Count == 0)
                {
                    ...
                    var mapping = Unsafe.As<IMapping>(handlersMap);
                    ...
                    if (set.Count == 0) _ = recipientsMap.TryRemove(key);
                    ...
                    if (handlersMap.Count == 0) _ = typesMap.TryRemove(mapping.TypeArguments);
                }
            }
        }
        finally
        {
            ...
            if (lockTaken) Monitor.Exit(recipientsMap);
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        lock (recipientsMap)
        {
            if (typeof(TToken) == typeof(Unit))
            {
                ...
                if (!TryGetMapping<TMessage>(out var mapping)) return;

                Recipient key = new(recipient);

                // Remove the handler (there can only be one for the unit type)
                if (!mapping.TryRemove(key)) return;
                ...
                if (mapping.Count == 0) _ = typesMap.TryRemove(mapping.TypeArguments);

                var set = recipientsMap[key];
            ...
            {
                // Get the registration list, if available
                if (!TryGetMapping<TMessage, TToken>(out var mapping)) return;

                Recipient key = new(recipient);

                if (!mapping.TryGetValue(key, out var dictionary)) return;
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        lock (recipientsMap)
        {
            ...
            {
                // Check whether there are any registered recipients
                if (!TryGetMapping<TMessage, TToken>(out var mapping)) goto End;
        ...
        where TToken : IEquatable<TToken>
    {
        lock (recipientsMap)
        {
            ...
            if (typeof(TToken) == typeof(Unit))
            {
                ...
                var underlyingMapping = GetOrAddMapping<TMessage>();
                ref var registeredHandler = ref underlyingMapping.GetOrAddValueRef(key);

                if (registeredHandler is not null) ThrowInvalidOperationExceptionForDuplicateRegistration();
            ...
            else
            {
                ...
                var underlyingMapping = GetOrAddMapping<TMessage, TToken>();
        ...
        where TMessage : class
    {
        ...
        if (typesMap.TryGetValue(key, out var target))
        {
    ...
    {
        Type2 key = new(typeof(TMessage), typeof(Unit));
        ref var target = ref typesMap.GetOrAddValueRef(key);
