...
public static partial class IMessengerExtensions
{
        ...
        where TToken : IEquatable<TToken>
    {
        ...
        static Action<IMessenger, object, TToken> LoadRegistrationMethodsForType(Type recipientType)
        {
            if (recipientType.Assembly.GetType("CommunityToolkit.Mvvm.Messaging.__Internals.__IMessengerExtensions") is
                ...
                extensionsType.GetMethod("CreateAllMessagesRegistratorWithToken", new[] { recipientType }) is MethodInfo
                    methodInfo)
            {
                var genericMethodInfo = methodInfo.MakeGenericMethod(typeof(TToken));
        ...
        static Action<IMessenger, object, TToken> LoadRegistrationMethodsForTypeFallback(Type recipientType)
        {
            ...
            if (registrationMethods.Length == 0) return static (_, _, _) => { };

            // Input parameters (IMessenger instance, non-generic recipient, token)
            var arg0 = Expression.Parameter(typeof(IMessenger));
            var arg1 = Expression.Parameter(typeof(object));
            var arg2 = Expression.Parameter(typeof(TToken));

            // Declare a local resulting from the (RecipientType)recipient cast
            var inst1 = Expression.Convert(arg1, recipientType);
        ...
        var registrationAction = DiscoveredRecipients<TToken>.RegistrationMethods.GetValue(
            recipient.GetType(),
            [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
            static (t) => LoadRegistrationMethodsForType(t));
