...
namespace FrameworkAgnostic.ComponentModel;
...
public abstract class ObservableValidator : ObservableObject, INotifyDataErrorInfo
{
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetProperty<T>([NotNullIfNotNull(nameof(newValue))] ref T field, T newValue, bool validate,
        [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(propertyName);

        var propertyChanged = SetProperty(ref field, newValue, propertyName);

        if (propertyChanged && validate) ValidateProperty(newValue, propertyName);
    ...
    /// </exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetProperty<T>([NotNullIfNotNull(nameof(newValue))] ref T field, T newValue,
        IEqualityComparer<T> comparer, bool validate, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(propertyName);

        var propertyChanged = SetProperty(ref field, newValue, comparer, propertyName);
        ...
        [CallerMemberName] string propertyName = null!)
    {
        ...
        var propertyChanged = SetProperty(oldValue, newValue, callback, propertyName);
        ...
        bool validate, [CallerMemberName] string propertyName = null!)
    {
        ...
        var propertyChanged = SetProperty(oldValue, newValue, comparer, callback, propertyName);
        ...
        where TModel : class
    {
        ...
        var propertyChanged = SetProperty(oldValue, newValue, model, callback, propertyName);
        ...
        where TModel : class
    {
        ...
        var propertyChanged = SetProperty(oldValue, newValue, comparer, model, callback, propertyName);
    ...
    public IEnumerable<ValidationResult> GetErrors(string? propertyName = null)
    {
        ...
        if (this.errors.TryGetValue(propertyName!, out var errors)) return errors;
    ...
    protected void ValidateAllProperties()
    {
        ...
        static Action<object> GetValidationActionFallback(Type type)
        {
            ...
            if (validatableProperties.Length == 0) return static _ => { };

            // MyViewModel inst0 = (MyViewModel)arg0;
            var arg0 = Expression.Parameter(typeof(object));
            var inst0 = Expression.Convert(arg0, type);

            // Get a reference to ValidateProperty(object, string)
            var validateMethod = typeof(ObservableValidator).GetMethod(nameof(ValidateProperty),
    ...
    /// <exception cref="Properties.Polyfills.ArgumentNullException">
    ///     Thrown when <paramref name="propertyName" /> is
    ///     <see langword="null" />.
    /// </exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected internal void ValidateProperty(object? value, [CallerMemberName] string propertyName = null!)
    {
        ...
        var isValid = Validator.TryValidateProperty(value, validationContext, propertyErrors);
        ...
        if (isValid)
        {
            if (errorsChanged)
            {
                ...
                if (totalErrors == 0) OnPropertyChanged(HasErrorsChangedEventArgs);
            }
        }
        else if (!errorsChanged)
        {
            ...
            if (totalErrors == 1) OnPropertyChanged(HasErrorsChangedEventArgs);
        }
        ...
        out IReadOnlyCollection<ValidationResult> errors)
    {
        ...
        var hasErrors = propertyErrors.Count > 0;
        ...
        var isValid = Validator.TryValidateProperty(value, validationContext, localErrors);
    ...
    private void ClearAllErrors()
    {
        ...
        foreach (var propertyInfo in errors)
        {
            ...
            if (hasErrors) ErrorsChanged?.Invoke(this, new DataErrorsChangedEventArgs(propertyInfo.Key));
        }
    ...
    private string GetDisplayNameForProperty(string propertyName)
    {
        ...
        _ = DisplayNamesMap.GetValue(GetType(), static t => GetDisplayNames(t))
            .TryGetValue(propertyName, out var displayName);
